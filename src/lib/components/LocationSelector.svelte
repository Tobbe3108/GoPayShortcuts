<script lang="ts">
  ...
  $effect(() => {
    componentSelectedLocationId = selectedLocation?.kitchenId?.toString() || ""; // If undefined, use ""
  });
  ...
  function handleUserSelectionChange() {    const newSelectedLocationObject = componentSelectedLocationId
      ? locations.find((l: Location) => l.kitchenId.toString() === componentSelectedLocationId)
      : undefined;
...
<select
  ...
  class="block w-full py-2 pl-3 pr-10 mt-1 text-base border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"  bind:value={componentSelectedLocationId}
