---
name: hono-routing
description: Build type-safe APIs with Hono for Cloudflare Workers, Deno, Bun, Node.js. Routing, middleware, validation (Zod/Valibot), RPC, streaming (SSE), WebSocket, security (CSRF, secureHeaders).
license: MIT
compatibility: agents
metadata:
  audience: backend-developers, full-stack-engineers, api-developers
  domain: api-development, backend-development
---
...
Provide production-ready patterns for building type-safe APIs with Hono, including routing, middleware chains, request validation, RPC patterns, and advanced features like streaming and WebSocket support.

## When to use me

Use this skill when:
- Building Hono APIs for any JavaScript runtime
- Implementing request validation with Zod/Valibot
- Setting up type-safe RPC client/server patterns
- Creating streaming endpoints (SSE, AI responses)
- Implementing WebSocket connections
- Configuring security middleware (CSRF, headers)
